...
const logger = winston.createLogger({
    ...
    transports: [
        new winston.transports.File({
            filename: 
            `logs/progress-${new Date().toISOString().slice(0, 10)}.log`,
...
logger.info = function() {
    info(Array.from(arguments).map(arg => arg.toString()).join(' '));
}


fs.readFile('./examples/index.html', 'utf8', async (err, content) => {
    if(err) {
        logger.error(err);
        return;
    }
...
async function addScripts(scripts, resourcePath) {
   ...
   const scriptsFullPath = path.join(
	   		   !converterConfig.useHooks ?
	   		    publicB : srcB, 'assets', 'script' +
	   		   ['', 's'].at(useScripts.length > 1));
   ...
   await Promise.all(useScripts.map(async (script) =>
  	 await fsp
		 .writeFile(path.join(scriptsFullPath, script.name),
		  	    script.content)));
   }
...
async function emplaceInRoot(scripts, scriptsFullPath, resourcePath) {
  ...
  const scriptsList = scripts.reduce((acc, script) => {
      if(!script.isInline) {
       	  const attrs = joinAttrs(getAttributesRaw(script.script), 
		  		  {src: script.name});
...
async function emplaceHooks(scripts, scriptsFullPath, resourcePath) {
  const scriptBasename = path.basename(scriptsFullPath);
  const scriptsList = scripts.reduce((acc, script) =>
	  acc + "'" + (!script.isInline ? script.name :
	  	path.join(scriptBasename, script.name)) + "'" + ',\n\t', '\n\t');
...
async function emplaceLinksOrMetasImpl(linksOrMetas, isLink, processingParams) {
   ...
   const stringLinksOrMetas = finalList.map(current => {
	const joint = joinAttrs(current);
	return tag + joint + '/>';
   }).reduce((cur, linkOrMeta) => cur + linkOrMeta + '\n', '\n');

   const { rootB } = processingParams;
   await emplaceImpl(isLink ? LINK_TAG : META_TAG, 
	   	     rootB, rootB, stringLinksOrMetas);
}
...
function overrideSet(standard, given) {
   ...
   given.forEach(entry => {
      if(entry?.name) finalSet[visibilityMap.get(entry.name)] = entry;
      else finalSet.push(entry);
   });
...
async function emplaceImpl(tag, readPath, writePath, replacement) {
   ...
   const toBeWritten =  content.toString()
	   	.replace(new RegExp(`@\{${tag}\}`, 'gm'), replacement);
...
function clip(str, maxLen) {
  ...
  return clippedStr.length === str.length ? clippedStr 
		: clippedStr + '...';
}
...
async function initializeProjectStructure() {
 ...
 // Duplicate the template in ./templates/javascript/react-app
   ...
   const templateFullPath = fullPathOf(path.join('templates', 
...
function randomCounter(nDigits) {
   return Math.floor((Math.random() + .001) * 0xdeadbeef)
		.toString(nDigits).slice(0, nDigits);
}
...
function modifyAttributes(node, attributes) {
	...
	if(reactAttributesLookup[attr])
	{
	    ...
	    switch(selection.type) {
	        case 'boolean':
		    node.setAttributeNS(null, selection.react,
...
function getAttributesRaw(node) {
    const attributeNodeArray = Array.prototype.slice.call(node.attributes);
    return attributeNodeArray.reduce((attrs, attribute) =>
	({...attrs, [attribute.name.indexOf(REPL_ID) != -1 ?
...
function formatStyle(value) {
    ...
    const allStyles = stylist.map(style => {
	...
	if(isEmpty(oneMatches))
        ...
        return `${jointOne.str + 
		oneMatches[oneMatches.length - 1]
	       .input.substring(jointOne.start)}: \`${other}\``; 
...
var metaTags = [
  ...
  {
     name: 'description',
     content: 'Web site created using jsx-generator'
  }
];
